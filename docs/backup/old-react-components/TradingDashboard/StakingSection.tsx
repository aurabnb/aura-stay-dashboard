...
import { getStakingMetrics, believeAPI } from '@/api/believe';
...
const StakingSection: React.FC = () => {
  ...
  const [stakingRewards, setStakingRewards] = useState(0); // User's pending rewards
  const [burnMetrics, setBurnMetrics] = useState<any>(null); // Real burn data
  const [isStaking, setIsStaking] = useState(false);
  const [isUnstaking, setIsUnstaking] = useState(false);
  const { toast } = useToast();

  // Fetch real burn and staking metrics
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        const metrics = await getStakingMetrics();
